...
String game_outcome = ""; //initialize empty
int game_number = 1; //initialize game as 1
unsigned long game_duration = 0;
...
void setup() {
  ...
  tft.reset();
  //uint16_t ID = 0x7793; //manually specify identifier of TFT
  tft.begin(0x6809); //get ready
...
{  
   ...
   tft.setTextColor(WHITE);
   tft.setTextSize(2);
   tft.print("Game " + String(game_number)); 
   //Draw frame
   tft.drawRect(0,0,319,240,WHITE);
...
{
  game_duration = timeStop - timeStart;
  tft.fillScreen(BLACK);
...
void saveData()
{
  ...
  { // check the card is still there
    ...
    if (sensorData)
    {
      sensorData.println(game_number + "," + game_outcome + "," + game_duration);
      sensorData.close(); // close the file
    }
  }
}
